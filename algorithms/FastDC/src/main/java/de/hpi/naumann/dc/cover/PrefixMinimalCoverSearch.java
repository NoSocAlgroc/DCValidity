...
public class PrefixMinimalCoverSearch implements IMinimalCoverSearch {
	...
	public PrefixMinimalCoverSearch(PredicateBuilder predicates2, TranslatingTreeSearch tree) {
		for (Collection<Predicate> pSet : predicates2.getPredicateGroups()) {
			IBitSet bitset = LongBitSet.FACTORY.create();
			for (Predicate p : pSet) {
				bitset.or(PredicateSetFactory.create2(p).getBitset());
			}
	...
	public Collection<IBitSet> getBitsets(IEvidenceSet evidenceSet) {
		...
		log.info("Sorting new bitsets..");
		 sortedNegCover = minimalize(sortedNegCover);
	...
	public DenialConstraintSet discoverDenialConstraints(IEvidenceSet evidenceSet) {
		...
		getBitsets(evidenceSet).forEach(valid -> {
			set.add(new DenialConstraint(PredicateSetFactory.create(valid).convert()));
		});
...
//	@SuppressWarnings("unused")
	private List<IBitSet> minimalize(final List<IBitSet> sortedNegCover) {
