...
public class Input {
	...
	private final List<ColumnPair> pairs;
	public String name;
	...
	public Input(RelationalInput relationalInput, int rowLimit) throws InputIterationException {
		...
		pairs = new ArrayList<ColumnPair>();
		for (int i = 0; i < getColumns().length; ++i) {
			ParsedColumn<?> c1 = getColumns()[i];
			for (int j = i; j < getColumns().length; ++j) {
//				 if (pairs.size() > 10)
//				 break;
				ParsedColumn<?> c2 = getColumns()[j];
				boolean joinable = c1.isJoinable(c2);
				boolean comparable = c1.isComparable(c2);
