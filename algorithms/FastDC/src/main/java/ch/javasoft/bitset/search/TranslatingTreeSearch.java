...
public class TranslatingTreeSearch implements ITreeSearch {
	...
	public void forEachSubSet(IBitSet bitset, Consumer<IBitSet> consumer) {
		search.forEachSubSet(translator.bitsetTransform(bitset), superset -> consumer.accept(translator.bitsetRetransform(superset)));
	}

	@Override
	public void forEachSuperSet(IBitSet bitset, Consumer<IBitSet> consumer) {
		search.forEachSuperSet(translator.bitsetTransform(bitset), superset -> consumer.accept(translator.bitsetRetransform(superset)));
	}
	...
	public Collection<IBitSet> handleInvalid(IBitSet invalidDCU) {
		...
		Collection<IBitSet> added = new ArrayList<>();
				...
				if (temp.isEmpty()) {
					...
					for (int i = valid.nextSetBit(0); i >= 0; i = valid.nextSetBit(i + 1)) {
						...
						if (!search.containsSubset(add)) {
							search.add(add);
							added.add(translator.bitsetRetransform(add));
						}
					}
				}
			}
	...
	public boolean contains(IBitSet bitset) {
		return search.contains(translator.bitsetTransform(bitset));
	}
}
