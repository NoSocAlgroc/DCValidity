...
public class MinimalCoverSearch implements IMinimalCoverSearch {
	...
	public DenialConstraintSet getDenialConstraints(IEvidenceSet evidenceSet) {
		...
		for (PredicateSet s : MC) {
			...
			DenialConstraint dc=new DenialConstraint(s);
			dcs.add(dc);
			double count=0;
			for(PredicateSet e:evidenceSet){				

				boolean satisfied=false;
				for(Predicate p:s){
					satisfied=satisfied ||  !e.containsPredicate(p);
				}
				if(satisfied)count=count+1.f;
			}
			count=count/evidenceSet.size();
			dc.setConfidence(count);
			dc.setSupport(0.5f);
